            ...
            >
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-sm md:text-base text-gray-500">Date: {formatDate(order?.created_at)}</p>
                </div>

                <div className="flex gap-4">

                {(order?.status === "in_process" || order?.status === "pending") && <Button className="bg-red-500 p-2 md:px-4 text-xs md:text-sm font-medium rounded-lg hover:bg-red-600" onClick={() => mutateAsync(order?.id)}>Cancel Order</Button>}
                <span
                  className={` p-2 md:px-4 text-xs md:text-sm font-medium rounded-lg ${getStatusStyles(order?.status)}`}
                  >
                  {order?.status}
                </span>
              ...
              <div className="mb-4">
                {order?.items?.map((item, index) => (
                  ...
                  >
                    <p className="font-medium text-sm md:text-base">{item?.product_name} (x{item?.quantity})</p>
