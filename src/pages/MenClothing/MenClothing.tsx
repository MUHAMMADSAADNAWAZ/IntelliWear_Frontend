...
const MenClothing = () => {
  return (
    ...
    <h1 className="text-4xl text-center font-bold text-yellow-500 mb-6">Men’s Style Picks</h1>    
      
      <h2 className="text-2xl font-semibold text-blue-500 mb-4">Clothes</h2>
      <div className="grid grid-cols-4 gap-4 mb-4">
        {clothesData.filter(item => item.category === "men" || item.category === "all").map((element, index) => (
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        {footwearData.filter(element => element.category === "men" || element.category === "all").map((element, index) => (
          <ClothInfo key={index} img={element.img} name={element.name} price={element.price} id={element.id} product={element} />
        ))}
      </div>
    ...
    <div className="grid grid-cols-4 gap-4">
      {accessoriesData.filter(clothing => clothing.category === "men" || clothing.category === "all").map((element, index) => (
        <ClothInfo key={index} img={element.img} name={element.name} price={element.price} id={element.id} product={element} />
      ))}
    </div>
