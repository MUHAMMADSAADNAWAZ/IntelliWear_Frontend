...
<div className="flex flex-col justify-between  p-6 max-h-[82svh] w-4/5 ">
  <MainContainer className="rounded-xl shadow-md overflow-y-hidden">
    <ChatContainer className="">
      <h2 className="text-xl font-bold text-indigo-900 mb-4">Knowledge Assistant</h2>
      <MessageList
        className="h-[calc(100vh-380px)] overflow-y-auto pr-2 pb-4"
        scrollBehavior="smooth"
      >
        {messages?.map((item, i) => (
          <div 
            key={i}
            className={`mb-6 mt-3 ${item?.sender === "gemini" ? "flex gap-4" : "flex justify-end"}`}
          >
            ...
            {item?.sender === "gemini" && item?.text && (
              <div className="flex flex-col w-3/5  p-3 rounded-3xl font-normal text-base text-[#141B34]">
                ...
                <Message
                  model={{ ...item, type: "custom", direction: item.sender === "gemini" ? "incoming" : "outgoing", position: "normal" }}
            ...
            {item?.sender !== "gemini" && (
              <div className="max-w-[60%]">
                <Message
                  model={{ ...item, type: "custom", direction: item.sender === "user" ? "outgoing" : "incoming", position: "normal" }}
  ...
  <div className="mt-6 flex flex-col items-center w-full">
    ...
    <p className="text-center text-gray-500 text-xs font-medium mt-3">
      Bot responses may not always be accurate. Use the information at your own discretion.
    </p>
  </div>
</div>
