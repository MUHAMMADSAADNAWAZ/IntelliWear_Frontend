      ...
      const response = await GeminiApi.post(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${import.meta.env.VITE_GEMINI_KEY}`,
        {
          ...
          system_instruction: {
            parts: [
              {
                text: user?.user_info?.user_type === "customer" ? "You are an expert in IntelliWear’s company policies, including return policy, payment security, and delivery process. You will only answer user queries related to these policies based on the provided company guidelines. If you do not have relevant information, ask the user to check the official policy page. Do not answer unrelated questions." : "You are an expert in e-commerce product naming , description generation , size and price. You will assist the admin in creating compelling product names and detailed descriptions that highlight key features, materials, and benefits. Ensure that the descriptions are engaging, SEO-friendly, and aligned with current fashion trends. You will only respond to product-related queries and will not provide any other information. You will give information about products according to Pakistani market." ,
              },
            ],
          },
        },
